 ...
 */
int ngx_http_enable_fast_start(ngx_str_t *path, ngx_fd_t ngx_open_file_cached_fd,
    ...
    //    // we will never run in to this
    //    if (ngx_open_file_cached_fd < 0) {
     ...
     * at the end */
    while (1) {
        if (read(ngx_open_file_cached_fd, atom_bytes, ATOM_PREAMBLE_SIZE) == 0) break;
        ...
        if (atom_type == FTYP_ATOM) {
            ...
            if (!ftyp_atom) {
                ngx_log_error(NGX_LOG_ERR, log, ngx_errno, "could not allocate %"PRIu64" byte for ftyp atom\n",
                        atom_size);
                goto error_out;
            }
            lseek(ngx_open_file_cached_fd, -ATOM_PREAMBLE_SIZE, SEEK_CUR);
            ngx_log_debug(NGX_LOG_DEBUG, log, 0, "atom_size: %"PRIu64" \n", atom_size);
            if (read(ngx_open_file_cached_fd, ftyp_atom, atom_size) < 0) {
                perror((const char *) path->data);
                goto error_out;
            }
            start_offset = atom_size;
            ngx_log_debug(NGX_LOG_DEBUG, log, 0, "start_offset to verify: %"PRIu64" \n", start_offset);
        } else {
            /* 64-bit special case */
            if (atom_size == 1) {
                ...
                lseek(ngx_open_file_cached_fd, atom_size - ATOM_PREAMBLE_SIZE * 2, SEEK_CUR);
            } else {
                lseek(ngx_open_file_cached_fd, atom_size - ATOM_PREAMBLE_SIZE, SEEK_CUR);
            }
        }
        ngx_log_debug(NGX_LOG_DEBUG, log, 0, "%c%c%c%c %10"PRIu64" %"PRIu64"\n",
                (atom_type >> 24) & 255,
                (atom_type >> 16) & 255,
                (atom_type >> 8) & 255,
                (atom_type >> 0) & 255,
        ...
        if ((atom_type != FREE_ATOM) &&
                ...
                (atom_type != UUID_ATOM) &&
                (atom_type != FTYP_ATOM)) {
            ngx_log_error(NGX_LOG_ERR, log, ngx_errno, "encountered non-QT top-level atom (is this a Quicktime file?)\n");
            break;
        }
    ...
    if (atom_type != MOOV_ATOM) {
        ngx_log_debug(NGX_LOG_DEBUG, log, 0, "last atom in file was not a moov atom\n");
        if (ftyp_atom) ngx_pfree(r->connection->pool, ftyp_atom);
        // dont close it, not our job
        //        close(infile_fd);
        return NGX_OK;
    }
    ...
    if (!moov_atom) {
        ngx_log_error(NGX_LOG_ERR, log, ngx_errno, "could not allocate %"PRIu64" byte for moov atom\n",
                atom_size);
        goto error_out;
    }
    ...
    if (BE_32(&moov_atom[12]) == CMOV_ATOM) {
        ngx_log_error(NGX_LOG_ERR, log, ngx_errno, "this utility does not support compressed moov atoms yet\n");
        free(ftyp_atom);
    ...
    if (!temp_buf) {
        ngx_log_error(NGX_LOG_ERR, log, ngx_errno, "Cannot allocate %"PRIu64" byte for temp buf \n", moov_atom_size);
        goto error_out;
    }
    ...
    for (i = 4; i < moov_atom_size - 4; i++) {
        ...
        if (atom_type == STCO_ATOM) {
            ngx_log_debug(NGX_LOG_DEBUG, log, 0, " patching stco atom...\n");
            ...
            ngx_log_debug(NGX_LOG_DEBUG, log, 0, " patching co64 atom...\n");
            atom_size = BE_32(&moov_atom[i - 4]);
            if (i + atom_size - 4 > moov_atom_size) {
                ngx_log_error(NGX_LOG_ERR, log, ngx_errno, " bad atom size\n");
                goto error_out;
    ...
    ngx_log_debug(NGX_LOG_DEBUG, log, 0, "outfile fd: %d\n", outfile_fd);
    if (outfile_fd < 0) {
        perror((const char *) path->data);
        goto error_out;
    }
    ...
    if (ftyp_atom_size > 0) {
        ngx_log_debug(NGX_LOG_DEBUG, log, 0, " writing ftyp atom...\n");
     ...
     */
    ngx_log_debug(NGX_LOG_DEBUG, log, 0, " moov_atom_size: %"PRIu64" \n", moov_atom_size);
    while (last_offset) {
